          ...
          data.ideas.forEach(contract => {
            ...
            if (!contractMap.has(contract.session_id)) {
              ...
              contractMap.set(contract.session_id, {
                ...
                evaluationScore: contract.metadata?.evaluation_score || contract.evaluation_score || contract.ai_score || null,
              ...
              >
                <div className="p-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-500">
                    ...
                    {(contract.evaluationScore || contract.aiScore) && (
                      <div className="flex justify-between">
                        <span>Score:</span>
                        <span className={`font-medium ${(contract.evaluationScore || contract.aiScore) >= 70 ? 'text-green-600' : (contract.evaluationScore || contract.aiScore) >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>
                          {(contract.evaluationScore || contract.aiScore)}/100
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full sm:p-6">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm text-gray-500">
                      ...
                      {(selectedContract.evaluationScore || selectedContract.aiScore) && (
                        <div>
                          <span className="font-medium">Score:</span> 
                          <span className={`ml-1 ${(selectedContract.evaluationScore || selectedContract.aiScore) >= 70 ? 'text-green-600' : (selectedContract.evaluationScore || selectedContract.aiScore) >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>
                            {(selectedContract.evaluationScore || selectedContract.aiScore)}/100
                          </span>
                        </div>
                      )}
                    </div>
